...
public class BoardTile extends Tile{	
	...
	//serve, dopo aver verificato se una tile può essere presa, per capire quali altre tile possono essere prese insieme a lei
	// il senso è quello di creare una lista di opzioni tra le quali il giocatore può scegliere quali gruppi di tile prendere
	public ArrayList<BoardTile> Choices () {
		...
		if (this.canBePickedUp()) {
			if((this.tileRigth().canBePickedUp()) && (this.tileRigth().getType().equals(this.getType()))) {
				if((this.tileRigth().tileRigth().canBePickedUp()) && (this.tileRigth().tileRigth().getType().equals(this.getType()))) {
					choices.add(this);
				...
				choices.add(this);// si creeranno dei duplicati della prima tile, ma servono per distinguere i vari gruppi che possono essere scelti, poichè si possono prendere solo tile adiacenti
			}
			if((this.tileDown().canBePickedUp()) && (this.tileDown().getType().equals(this.getType()))) {//non mi è venuto in mente come ottimizzare il ciclo perciò ho semplicemente reiterato lo stesso processo più volte
				if((this.tileDown().tileDown().canBePickedUp()) && (this.tileDown().tileDown().getType().equals(this.getType()))) {
					choices.add(this);
			...
			if((this.tileLeft().canBePickedUp()) && (this.tileLeft().getType().equals(this.getType()))) {
				if((this.tileLeft().tileLeft().canBePickedUp()) && (this.tileLeft().tileLeft().getType().equals(this.getType()))) {
					choices.add(this);
			...
			if((this.tileUp().canBePickedUp()) && (this.tileUp().getType().equals(this.getType()))) {
				if((this.tileUp().tileUp().canBePickedUp()) && (this.tileUp().tileUp().getType().equals(this.getType()))) {
					choices.add(this);
